...
/// Pick a backend by fingerprinting the system. Order, mirroring
/// Tailscale's "Sisyphean DNS in Linux" detection:
///
/// 1. **Magic-comment fingerprint** on `/etc/resolv.conf`. resolved
///    leaves `# This is /run/systemd/resolve/...managed by man:systemd-
///    resolved(8).`; resolvconf leaves `# Generated by resolvconf`;
///    NetworkManager leaves `# Generated by NetworkManager`. The
///    comment is more reliable than a symlink check because some
///    setups write a real file rather than symlinking the stub.
/// 2. **D-Bus probe** for resolved when the comment is ours or absent.
///    A bus connection isn't enough — we introspect the resolve1
///    object to confirm the daemon is actually answering.
/// 3. **Direct file fallback** for everything else, with a `warn!`
///    naming the manager we'd have wanted (so a future contributor
///    can wire that backend without having to repeat the detection
///    work).
async fn detect_backend() -> Backend {
    ...
    match signal {
        ResolvSignal::SystemdResolved | ResolvSignal::Unknown => {
            match ResolvedBackend::try_connect().await {
                Ok(b) => {
                    info!(
                        backend = "systemd-resolved",
                        fingerprint = ?signal,
                        "DNS backend selected"
                    );
                    return Backend::Resolved(b);
                }
                Err(e) => {
                    debug!(error = %e, "resolved D-Bus probe failed");
                }
            }
        }
        ResolvSignal::NetworkManager => {
            warn!(
                "/etc/resolv.conf is managed by NetworkManager — split-DNS via \
                 NetworkManager's D-Bus isn't implemented yet; falling back to \
                 direct file manipulation. Track this for L2."
            );
        }
        ResolvSignal::Resolvconf => {
            warn!(
                "/etc/resolv.conf is managed by resolvconf — split-DNS via \
                 resolvconf isn't implemented yet; falling back to direct \
                 file manipulation. Track this for L2."
            );
        }
    }
