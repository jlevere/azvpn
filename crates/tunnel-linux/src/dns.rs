...
trait Resolved {
    ...
    ///
    /// The explicit name is required: zbus derives `SetLinkDns` from
    /// `set_link_dns`, and resolved rejects that as an unknown method.
...
mod tests {
    ...
    enum ResolvedCall {
        SetLinkDns(i32, Vec<(i32, Vec<u8>)>),
        SetLinkDomains(i32, Vec<(String, bool)>),
        RevertLink(i32),
    }

    /// Stand-in for systemd-resolved. Each method carries resolved's
    /// real member name explicitly, so the test does not depend on
    /// zbus's snake_case → PascalCase conversion on this side.
    struct FakeResolved {
        calls: std::sync::Arc<std::sync::Mutex<Vec<ResolvedCall>>>,
    }
    ...
    impl FakeResolved {
        ...
        fn set_link_dns(&self, ifindex: i32, addresses: Vec<(i32, Vec<u8>)>) {
            let call = ResolvedCall::SetLinkDns(ifindex, addresses);
            self.calls.lock().unwrap().push(call);
        }

        #[zbus(name = "SetLinkDomains")]
        fn set_link_domains(&self, ifindex: i32, domains: Vec<(String, bool)>) {
            let call = ResolvedCall::SetLinkDomains(ifindex, domains);
            self.calls.lock().unwrap().push(call);
        }
        ...
        fn revert_link(&self, ifindex: i32) {
            self.calls
                .lock()
                .unwrap()
                .push(ResolvedCall::RevertLink(ifindex));
        }
    }

    #[tokio::test]
    async fn resolved_proxy_calls_resolved_member_names() {
        let calls = std::sync::Arc::default();
        let fake = FakeResolved {
        ...
        let server = zbus::connection::Builder::unix_stream(server_sock)
            ...
            .unwrap()
            .p2p()
            .serve_at("/org/freedesktop/resolve1", fake)
            .unwrap()
            .build();
        ...
        let proxy = ResolvedProxy::new(&client).await.unwrap();
        proxy
            .set_link_dns(7, vec![(libc::AF_INET, vec![10, 0, 0, 36])])
            .await
            .unwrap();
        proxy
            .set_link_domains(7, vec![("corp.example.com".to_string(), true)])
            .await
            .unwrap();
        proxy.revert_link(7).await.unwrap();

        assert_eq!(
            *calls.lock().unwrap(),
            [
                ResolvedCall::SetLinkDns(7, vec![(libc::AF_INET, vec![10, 0, 0, 36])]),
                ResolvedCall::SetLinkDomains(7, vec![("corp.example.com".to_string(), true)]),
                ResolvedCall::RevertLink(7),
            ]
        );
    }
