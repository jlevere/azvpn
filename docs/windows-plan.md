...
| W4 — `azvpn up` end-to-end | **shipped** | The original TLS_CHANNEL_BUF_SIZE theory was wrong — clock skew on win-test-vm was rejecting our tls-auth HMAC. MS Azure VPN Client's own "Test Application Prerequisites" diagnostic flagged it; `w32tm /resync` + correct NTP source fixed the reset loop. See `feedback_azure_hmac_replay_clockskew` memory |
| W5 — NRPT split-DNS | **shipped** | Tailscale's `nrpt_windows.go` model: registry-direct rules under `HKLM\SYSTEM\...\DnsPolicyConfig`, GUID-keyed, GP-auto-detect, dnscache running probe. End-to-end verified on win-test-vm: `Resolve-DnsName internal.example.com... → <smoke-endpoint-ip>` |
...
| W6.6 — `if-watch` Windows verification | **shipped** | Daemon log confirms reachability events fire on adapter Up/Down on win-test-vm. Wall-clock-jump path covers sleep/resume cleanup |
...
- Tested on Windows 10 22H2 and Windows 11 23H2 (win-test-vm). Older
...
`core::route` path unchanged. Verify on win-test-vm.
...
to work as-is — verify on win-test-vm, no new code.
...
Each phase has a concrete acceptance test against win-test-vm.
...
**Acceptance:** on a fresh win-test-vm box,
...
on win-test-vm brings up a Wintun interface (visible in
...
**Acceptance:** `azvpn connect` on win-test-vm brings the
...
adapter / link changes on win-test-vm (smoke-test:
...
## 5. Test loop on win-test-vm

`win-test-vm` is a Windows Server 2025 Domain Controller (AD
...
```
ssh -J lab-bastion localuser@<win-test-ip>
```

- `lab-bastion` resolves via `~/.ssh/config` (Tailscale `<lab-bastion-tsip>`,
  user `debian`, key `~/.ssh/azvpn-lab-bastion`).
...
Known wart in `~/.ssh/config`: the `Host win-test-vm` block has
...
```pwsh
# In an admin PowerShell on win-test-vm:
...
```sh
# Build natively on win-test-vm — no cross-compile.
ssh -J lab-bastion localuser@<win-test-ip> \
...
| 3 | OpenVPN config quirks on Windows (`route-method exe` vs `ipapi`, `--block-outside-dns` push semantics) | Compare emitted config vs official-client capture on Windows; smoke-test on win-test-vm before W4 acceptance. |
