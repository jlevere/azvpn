...
| Phase | State | Notes |
|---|---|---|
| W0 — workspace deps + module skeletons | **shipped** | `cargo check --target x86_64-pc-windows-msvc --workspace` green on win-test-vm |
...
| W4 — `azvpn up` end-to-end | **blocked on openvpn version** | AAD device-code, token cache, RPC handoff, daemon→openvpn spawn, wintun adapter init, TCP connect to Azure gateway — all working. Server immediately resets the TCP connection after `HARD_RESET_CLIENT_V2` (state machine cycles RESOLVE → TCP_CONNECT → WAIT → reset, forever). macOS uses our patched openvpn 2.6.19+OpenSSL 3.6.1; Windows uses stock openvpn-community 2.6.14+OpenSSL 3.4.1. Versions diff by 5 minor releases + a meaningful OpenSSL gap, and we use a custom `TLS_CHANNEL_BUF_SIZE = 6144` patch on macOS. Next: build patched openvpn for Windows (W4a). internal-app smoke-test acceptance defers behind this. |
